...
Source: http://webdocs.cs.ualberta.ca/~darse/rsb-ts1.c 

Changes in comparison to original:

Line 245:
Was: int sum[1 + trials][3];
Is:  int sum[1 + TRIALS][3];

Lines 1347-1350:
Was: memset(moves0, 0, sizeof moves0);
     memset(moves1, 0, sizeof moves1);
     memset(moves2, 0, sizeof moves2);
     memset(moves3, 0, sizeof moves3);
Is:  memset((void*)moves0, 0, sizeof(moves0));
     memset((void*)moves1, 0, sizeof(moves1));
     memset((void*)moves2, 0, sizeof(moves2));
     memset((void*)moves3, 0, sizeof(moves3));

Lines 1022, 1028, 1065, 1077:

Were: if((node<0)||(node>=trie_size))fprintf(stderr, "OUCH\n");
Are:  /*if((node<0)||(node>=trie_size))fprintf(stderr, "OUCH\n"); * / <---this is actually a comment there
...
/* by Russ Williams (e-mail: russrocker at hotmail dot com */
    ...
    if (n_moves == 0) {
        memset((void*)moves0, 0, sizeof(moves0));
        memset((void*)moves1, 0, sizeof(moves1));
        memset((void*)moves2, 0, sizeof(moves2));
        memset((void*)moves3, 0, sizeof(moves3));
    } else {
